...
import { clientInvoiceModel } from "../../data/mongo/models/clientInvoice.model"; // Asegúrate de tener este modelo
import { DailyCashModel } from "../../data/mongo/models/dailyCash.model";
import { IClientInvoice } from "../../interfaces/IClientInvoice.interface";
...
export class ClientInvoiceController {
  ...
  getOneClientInvoice = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const clienteInvoice = await clientInvoiceModel.findOne(query);
      if (!clienteInvoice) {
  ...
  updateClientInvoice = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { ...rest } = req.body;
      const { organizationId } = req.user; // Obtener organizationId del usuario logueado
      ...
      const clienteInvoice: IClientInvoice = await clientInvoiceModel.findOne({
        _id: id,
        organizacion: organizationId,
      });
      if (!clienteInvoice) {
